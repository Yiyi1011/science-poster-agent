...
def validate_model_policy(settings: Settings):
    for model in (settings.qwen_text_model, settings.qwen_studio_model, settings.qwen_image_model, settings.qwen_tts_model, settings.qwen_vision_model):
...
def guard_text_budget(settings: Settings, reserve_cny=0.25):
    ...
    if total + reserve_cny >= min(settings.budget_pause_cny, settings.budget_limit_cny, 70):
